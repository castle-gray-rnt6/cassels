...
def cassels_form_filter(cyclotomic_integer_list):
    ...
    OUTPUT: A list of those elements which do not appear in one of Cassels's infinite families.
    """
    filtered = []
...
def minimal_weight(a):
    ...
    if b in QQ: # Don't sweat the easy stuff
        return ZZ(b).abs()
    pol = b.polynomial()
    # If the minimal level is not squarefree, split over the largest prime with a repeated factor, then recurse.
    ...
    # If the minimal level is squarefree, try splitting over a prime p to see if we get fewer than p/2 summands. If so, recurse.
    ...
    roots = K.roots_of_unity()
    # First check the power basis representations of b times a root of unity.
    nmin = Infinity
    for zeta in roots:
